**Overview**
- **Origin**: This project was forked from `Fladder` (https://github.com/DonutWare/Fladder). Replace the placeholders below with your new app name and logo when available.
...
- **Update packaging metadata**: update `AppImageBuilder.yml`, `flatpak/`, `snap/`, `Dockerfile`, CI workflows and badges in `README.md` that reference `DonutWare/Fladder`.
...
- `README.md` — marketing images and badges referencing `DonutWare/Fladder` and `assets/marketing` screenshots.
...
- CI / workflow files and badges (search for `DonutWare/Fladder` and update to your repo name).
...
- **Remove git history**: delete the `.git` folder to start a fresh repo (recommended if you don't want to keep commit history or references to DonutWare).  
